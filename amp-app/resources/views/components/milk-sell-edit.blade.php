                ...
                <div class="card-body">
                    <form method="POST" action="{{ $updateRoute }}">
                        ...
                        <div class="mt-4">
                            ...
                            <input class="form-control form-control-lg" type="text" name="sell_price" value="{{$data->sell_price}}" id="sell_price" >
                            @error('sell_price')
                                <code>{{ $message }}</code>
                            @enderror
                        </div>
                            ...
                            <label class="form-label" for="location_id">{{ __('Location') }}</label>
                                <select class="form-select" name="location_id" id="location_id">
                                    @foreach($locations as $loc)
                                        <option @if($loc->id == old('location_id', $data->location_id)) selected @endif value="{{$loc->id}}">{{$loc->name}}</option>
